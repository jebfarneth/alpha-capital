...
This entrypoint orchestrates the existing historical PIT universe
reconstruction and audited historical M4 replay one trading date at a time.
The replay implementation remains in ``HistoricalM4ReplayJob``; this runner
adds target guards, date-level checkpointing, and resumable/idempotent
orchestration for larger historical ranges.
"""
...
def _has_completed_m4_replay_evidence(session: Any, replay_day: date) -> bool:
    ...
    runs = (
        ...
        .join(EvidenceJob, EvidenceJob.job_id == EvidenceJobRun.job_id)
        .filter(
            EvidenceJob.job_name == HISTORICAL_M4_REPLAY_JOB_NAME,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Reconstruct historical daily M4 fired cohorts by running PIT "
            "universe reconstruction and audited M4 replay date by date."
        )
    )
