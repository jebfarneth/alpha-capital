...
def test_health_report_surfaces_m2_fatal_fetch_error_count(db_session):
    _add_feature(db_session, "feature-m2-metric", "M2M")
    _add_signal(db_session, "signal-m2-metric", "feature-m2-metric", "M2M")
    ...
    report = build_m4_health_report(
        ...
        next_execution_session="2026-05-26",
        run_timestamp="2026-05-26T08:00:00+00:00",
        m2_metrics={"fatal_fetch_error_count": 3},
    )

    assert report["m2_assembly"]["fatal_fetch_error_count"] == 3
    assert "fatal_fetch_error_count" not in report["health_verdict"]["checks"]
...
def test_main_enable_m1_failure_aborts_before_health_report(monkeypatch, capsys):
    monkeypatch.setenv("DATABASE_URL", "postgresql+psycopg://u:p@host/db")
    monkeypatch.delenv("ALPHA_DB_SCHEMA", raising=False)
    ...
    def fail_report(*_args, **_kwargs):  # pragma: no cover - must not be called
        raise AssertionError("health report must not build after fatal M1")
    ...
    assert rc == 7
    assert "M1 daily failed with exit 7" in capsys.readouterr().out


def test_main_enable_m2_tolerated_fetch_errors_reaches_healthy_report(
    monkeypatch,
    tmp_path,
):
    monkeypatch.setenv("DATABASE_URL", "postgresql+psycopg://u:p@host/db")
    ...
    def fake_m2(**kwargs):
        ...
        return run_nightly_canonical.RunInvocation(
            ...
            metrics={
                "fetch_error_count": 1,
                "fatal_fetch_error_count": 0,
                "tolerated_fetch_error_count": 1,
            },
        )
    ...
    def fake_build_report(session, **kwargs):
        ...
        assert kwargs["m2_metrics"]["fatal_fetch_error_count"] == 0
        return {
            "health": True,
            "run_metadata": {},
            "universe": {},
            "m4_assembly": {},
            "m1_assembly": {},
            "m2_assembly": {"fatal_fetch_error_count": 0},
            "m4_signals": {},
            "data_quality": {},
            "forward_return_guard": {},
            "forward_context_panel": {},
    ...
    assert report["health"] is True
    assert report["m2_assembly"]["fatal_fetch_error_count"] == 0


def test_main_enable_m2_fatal_fetch_error_aborts_before_health_report(
    monkeypatch,
    capsys,
):
    monkeypatch.setenv("DATABASE_URL", "postgresql+psycopg://u:p@host/db")
    ...
    def fake_m2(**kwargs):
        return run_nightly_canonical.RunInvocation(
            ...
            metrics={"fatal_fetch_error_count": 1},
        )

    def fail_report(*_args, **_kwargs):  # pragma: no cover - must not be called
        raise AssertionError("health report must not build after fatal M2")
...
def test_main_enable_m3_unhandled_exception_is_observability_only(
    monkeypatch,
    tmp_path,
):
    monkeypatch.setenv("DATABASE_URL", "postgresql+psycopg://u:p@host/db")
    ...
    def fake_m3(**kwargs):
        calls.append(("m3", kwargs))
        raise RuntimeError("injected M3 crash")
    ...
    def fake_build_report(session, **kwargs):
        ...
        assert kwargs["m1_run_id"] == "m1-run"
        assert kwargs["m2_run_id"] == "m2-run"
        assert kwargs["m3_run_id"] is None
        assert kwargs["m3_metrics"]["exit_code"] == 1
        assert kwargs["m3_metrics"]["run_status"] == "failed"
        assert kwargs["m3_metrics"]["errors"][0]["exception_type"] == "RuntimeError"
        return {
            "health": True,
            "run_metadata": {},
            "universe": {},
            ...
            "m3_assembly": {
                "exit_code": 1,
                "run_status": "failed",
                "errors": kwargs["m3_metrics"]["errors"],
            },
            "m4_signals": {},
            "data_quality": {},
            "forward_return_guard": {},
    ...
    report = json.loads(output.read_text())
    assert report["health"] is True
    assert report["m3_assembly"]["errors"][0]["message"] == "injected M3 crash"
